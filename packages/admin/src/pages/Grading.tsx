...
import { Card, Table, Input, InputNumber, Button, message, Space, Typography, Tag } from 'antd';
...
const taskColumns = (handleGrade: (taskId: string, values: { score: number; comment?: string }) => void) => [
...
const Grading: React.FC = () => {
  ...
  React.useEffect(() => {
    api.get('/exams').then((res) => setExams(res.data.items)).catch((err) => console.error('Failed to load exams:', err));
    fetchTasks();
  }, []);
  ...
  const handleGrade = React.useCallback(async (taskId: string, values: { score: number; comment?: string }) => {
    try {
  ...
  return (
    <div>
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        <Card title="我的阅卷任务">
          <Table rowKey="id" dataSource={tasks} loading={loading} columns={taskColumns(handleGrade)} size="small" />
        </Card>
      </Space>
    </div>
  );
