...
| **速率限制** | @nestjs/throttler | 全局 100 req/min，防暴力破解 |
| **前端管理端** | React 18 + Ant Design 5 + Vite 5 | TypeScript |
| **前端答题端** | React 18 + Vite 5 + DOMPurify | 轻量无 UI 库，XSS 安全过滤 |
...
│           └── stores/      # Zustand 状态
├── docker/
│   └── docker-compose.yml   # PostgreSQL + Redis (含健康检查)
├── start.sh                 # 一键启动脚本
...
| **考场监控** | 实时显示在线/已交卷人数，WebSocket 实时推送违规告警，强制收卷、延长作答 |
...
| **考前拍照** | 调用摄像头拍照存档 |
| **全屏锁定** | 进入考试强制全屏，退出记录违规 |
| **切屏检测** | 监测页面可见性，切屏记录违规 |
| **防泄密** | 禁用右键菜单、复制、粘贴 |
| **选项乱序** | 每题选项随机打乱显示 |
| **交卷确认** | 二次确认弹窗，提示未答题数 |
| **成绩展示** | 交卷后显示得分/总分/百分比/及格状态 |
| **移动端适配** | 响应式布局，答题卡折叠为浮层 |
| **XSS 安全** | DOMPurify 白名单过滤渲染内容 |
...
考生进入考试时，系统根据策略实时从题库中抽取符合条件的题目。抽题算法使用 Fisher-Yates 均匀洗牌。
